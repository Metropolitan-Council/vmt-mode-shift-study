...
Walking on mild hills is not that bad, but very steep hills are unpleasant. We apply a double weight to streets or portions of streets sloped more than 10%.
...
We treat each meter of elevation gain as equivalent to 25 flat meters when biking (or when walking a bike).
...
1. Due to an [upstream bug](https://github.com/Project-OSRM/osrm-backend/issues/6614) that was not fixed in the version of the OSRM profiles we forked our profiles from, ways tagged "proposed" were used for routing in bicycle and foot profiles. There are only 43 km of these in the region, mostly realignments of existing streets, so this is not expected to affect overall results. Merging the upstream fix from OSRM should be fairly simple in future work.