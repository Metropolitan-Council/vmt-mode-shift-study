...
1. Non-residential high-speed (>25 mph, either explicit or implicit) shared bike lanes that are tagged `cycleway:left=shared_lane` or `cycleway:right=shared_lane` are treated as if there were no bike infrastructure (unless they have actual bike lanes in the opposite direction)—which is probably not too far off from reality. This likely does not affect routing very much, as these would be treated as LTS 3 if they were handled as shared lanes, and most would be categorized as LTS 3 by the remaining logic anyhow.
