...
class BaseStep:
    ...
    def apply_step(self, expression: pd.Series, prev: pd.Series):
        self.df.loc[:, f"feasible_{self.mode}_shift"] = prev
    ...
    def get_step_statistics(self, prev: pd.Series):
        percent_shifts_before = len(self.df[(self.df['mode']=='Car') & (prev)]) / len(self.df[self.df['mode']=='Car']) * 100
        percent_shifts_after = len(self.df[(self.df['mode']=='Car') & (self.df[f'feasible_{self.mode}_shift'])]) / len(self.df[self.df['mode']=='Car']) * 100
        
        prev_vmt = self.df[(self.df["mode"] == "Car") & (prev)]["vmt"].sum()
    ...
    def get_map(self):
        ...
        print(temp["community"].value_counts())
        print(temp["community"])
...
class CategoricalStep(BaseStep):
    ...
    def get_step_statistics(self, mode: str, df: pd.DataFrame, prev: pd.Series):
