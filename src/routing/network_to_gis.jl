...
import OSRM.Toolchain: OSRMToolchain, get_geometry, get_node_ids
...
function main(raw_args)
    ...
    result = Vector{@NamedTuple{id::Int64, name::String, ref::String, lts::Int32, pedquality::String, weight::Int32, duration::Int32, distance::Float32, oneway::Bool,
    ...
    for (i, node, weight, duration, distance) in zip(
        ...
        for (i, class) in enumerate(toolchain.class_names)
            if ann.class_data[i]
                ...
                elseif startswith(class, "pedqual")
                    pedquality == "none" || error("Duplicate ped quality!")
                    pedquality = class[8:end]
                end
            end
        end
        ...
        push!(result, (id=i, name=something(name.name, ""), ref=something(name.ref, ""), lts=lts, pedquality=pedquality,
            weight=weight.weight, duration=duration,
